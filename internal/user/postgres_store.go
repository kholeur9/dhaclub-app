...
func (pu *PostgresUser) Update(dto UpdateResponse) (*User, error) {
	...
	row := pu.db.QueryRow(`UPDATE users SET email = COALESCE($1, email), username = COALESCE($2, username), updated_at = $3 WHERE id = $4 RETURNING id, email, username, updated_at`, dto.Email, dto.Username, dto.UpdatedAt, dto.ID)
	err := row.Scan(&user.ID, &user.Email, &user.Username, &user.UpdatedAt)
	if err != nil {
		return nil, err
	}
	return &user, nil
